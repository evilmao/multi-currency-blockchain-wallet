...
func SignCompact(key *PrivateKey,
	...
	return makeCompact(S256(), sig, key, hash, isCompressedKey)
}
...
func SignCanonical(key *PrivateKey,
	hash []byte, isCompressedKey bool) ([]byte, error) {
	for i := 0; i < 25; i++ {
		sig, err := signRFC6979(key, hash, i)
		if err != nil {
			return nil, err
		}

		compactSig, err := makeCompact(S256(), sig, key, hash, isCompressedKey)
		if err != nil {
			continue
		}
...
func nonceRFC6979(privkey *big.Int, hash []byte, nonce int) *big.Int {
	if nonce > 0 {
		...
		hs := sha256.New()
		hs.Write(append(hash, nonceBytes...))
		hash = hs.Sum(nil)
	}
