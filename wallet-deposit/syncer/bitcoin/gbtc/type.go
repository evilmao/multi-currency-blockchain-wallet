...
func (tx *Transaction) SetBytes(buf []byte) error {
	...
	if tx.DeserConf.WithTime {
		tx.Time, err = binarySerializer.Uint32(reader, littleEndian)
		if err != nil {
			return err
		}
	}
...
func NewOutput(address string, value uint64) *Output {
	scriptData := CreateP2PKHScript(address, 1)
	return &Output{
		Value:      value,
		Script:     hex.EncodeToString(scriptData),
